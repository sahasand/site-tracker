...
export default function AttentionRail({ items, id }: AttentionRailProps) {
  if (items.length === 0) {
    return (
      <div id={id} className="flex items-center gap-2 px-4 py-3 rounded-lg bg-emerald-50 border border-emerald-100">
        <svg className="w-5 h-5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <span className="text-sm font-medium text-emerald-700">All clear — no sites need attention</span>
      </div>
    )
  }
